...
{
  auto engine = CreateTrtInferCore("/workspace/models/depth_anything_v2_vits.engine");
...
TEST(depth_anything_test, trt_core_correctness)
{
...
TEST(depth_anything_test, trt_core_pipeline_correctness)
{
  auto model         = CreateModel();
  model->InitPipeline();
  auto [image] = ReadTestImages();
...
TEST(depth_anything_test, trt_core_pipeline_speed)
{
  auto model         = CreateModel();
  model->InitPipeline();
  auto [image] = ReadTestImages();
